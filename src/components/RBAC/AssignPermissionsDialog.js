...
export default function AssignPermissionsDialog({ open, onClose, role, refreshRole }) {
  ...
  useEffect(() => {
    if (open && role) {
      setLoading(true);
      Promise.all([
        getPermissions(fetchWithAuth),
        fetchRolePermissions(fetchWithAuth, role.id)
      ])
  ...
  const handleToggle = async (permId) => {
    ...
    try {
      if (isSelected) {
        await removePermission(fetchWithAuth, role.id, permId);
        setSelected(prev => prev.filter(id => id !== permId));
        showSnackbar("Permission removed");
      } else {
        await assignPermission(fetchWithAuth, role.id, permId);
