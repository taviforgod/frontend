...
export default function StatusTypesPage() {
  ...
  async function loadStatuses() {
    try {
      const data = fetchWithAuth
        ? await getStatusTypes(fetchWithAuth)
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editId) {
        if (fetchWithAuth) {
          await updateStatusType(fetchWithAuth, editId, fields);
        } else {
        ...
        const newStatus = fetchWithAuth
          ? await createStatusType(fetchWithAuth, fields)
          : await createStatusType(fields);
  ...
  async function handleDeleteConfirmed() {
    ...
    try {
      if (fetchWithAuth) {
        await deleteStatusType(fetchWithAuth, statusToDelete.id);
      } else {
