...
export default function MilestoneTemplates() {
  ...
  const onSubmit = async (data) => {
    try {
      if (editTemplate) {
        if (fetchWithAuth) {
          await updateMilestoneTemplate(fetchWithAuth, editTemplate.id, data);
        } else {
        ...
        if (fetchWithAuth) {
          await createMilestoneTemplate(fetchWithAuth, data);
        } else {
  ...
  const confirmDelete = async () => {
    try {
      if (fetchWithAuth) {
        await deleteMilestoneTemplate(fetchWithAuth, deleteId);
      } else {
