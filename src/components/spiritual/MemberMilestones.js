...
import { getMilestoneTemplates } from '../../services/milestoneService';
...
export default function MemberMilestones({ memberId, refreshKey }) {
  ...
  const canView = user?.permissions?.includes('view_milestone_records');
  const canCreate = user?.permissions?.includes('create_milestone_record');
  const canDelete = user?.permissions?.includes('delete_milestone_record');
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [r, t] = await Promise.all([
        fetchWithAuth ? getMilestonesByMember(fetchWithAuth, memberId) : getMilestonesByMember(memberId),
        fetchWithAuth ? getMilestoneTemplates(fetchWithAuth) : getMilestoneTemplates()
      ]);
      setRecords(r || []);
  ...
  }, [memberId, fetchWithAuth]);

  useEffect(() => { load(); }, [memberId, refreshKey, load]);

  const handleDelete = async (id) => {
    if (!canDelete) {
      setSnackbar({ open: true, message: 'No permission to delete.', severity: 'error' });
      return;
    }
    if (!window.confirm('Remove this milestone?')) return;
    try {
      if (fetchWithAuth) {
        await deleteMilestone(fetchWithAuth, id);
      } else {
  ...
  if (!canView) {
    return (
      <Box sx={{ mt: 2 }}>
        <Typography color="text.secondary">You do not have permission to view milestones.</Typography>
      </Box>
    );
  }
