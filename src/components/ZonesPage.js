...
export default function ZonesPage() {
  ...
  async function loadZones() {
    try {
      ...
      const data = fetchWithAuth ? await getZones(fetchWithAuth) : await getZones();
      setZones(data);
      setError("");
    } catch (err) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editId) {
        if (fetchWithAuth) {
          await updateZone(fetchWithAuth, editId, fields);
        } else {
        ...
        const newZone = fetchWithAuth
          ? await createZone(fetchWithAuth, fields)
          : await createZone(fields);
  ...
  async function handleDeleteConfirmed() {
    ...
    try {
      if (fetchWithAuth) {
        await deleteZone(fetchWithAuth, zoneToDelete.id);
      } else {
