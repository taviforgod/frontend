              ...
              {history.map((m, i) => (
                <Box key={i} sx={{ display: 'flex', justifyContent: 'space-between', gap: 2 }}>
                  <Typography variant="body2">
                    {DateTime.fromISO(m.date).toLocaleString(DateTime.DATE_MED)}
                  </Typography>
                  <Typography variant="body2" sx={{ fontWeight: 700 }}>
...
function HealthCard({ group, onView }) {
  ...
  const lastMeeting = group.last_meeting_date
    ? DateTime.fromISO(group.last_meeting_date).toLocaleString(DateTime.DATE_MED)
    : "—";
  // Multiply by 100 for percent
