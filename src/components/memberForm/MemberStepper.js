...
function getChangedFields(original = {}, updated = {}) {
  const normalize = (v) => {
    ...
    // Luxon DateTime-like objects: use toISODate if present
    if (v && typeof v.toISODate === 'function') return v.toISODate();
    return v;
  };
  ...
  keys.forEach((key) => {
    const a = normalize(original[key]);
    const b = normalize(updated[key]);
    if (String(a) !== String(b)) changed[key] = updated[key];
  });
  return changed;
}
      ...
      try {
        ...
        if (step === 1 && !isEditMode) {
          const [dupEmail, dupPhone] = await Promise.all([
            checkDuplicate('email', formValues.email, fetchWithAuth),
            checkDuplicate('contact_primary', formValues.contact_primary, fetchWithAuth),
          ]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        console.debug('MemberStepper: stableInitial keys=', Object.keys(stableInitial || {}));
        console.debug('MemberStepper: formValues keys=', Object.keys(formValues || {}));
        console.debug('MemberStepper: changedFields to send=', changedFields);

        // if only profile photo changed, allow upload even when changedFields empty
        if (Object.keys(changedFields).length === 0 && !profilePhoto) {
          showSnackbar('No changes to update.', 'info');
          setIsSubmitting(false);
          return;
        }

        // NOTE: verify the updateMember service signature — it should send JSON body with Content-Type
        member = await updateMember(fetchWithAuth, formValues.id, changedFields);
        showSnackbar('Member updated successfully!');
      } else {
        member = await createMember(fetchWithAuth, formValues);
        showSnackbar('Member created successfully!');
      }

      if (profilePhoto && typeof profilePhoto !== 'string') {
        const memberId = isEditMode ? formValues.id : member.id;
        await uploadProfilePhoto(fetchWithAuth, memberId, profilePhoto);
      }
