...
export default function Profile() {
  const { user, logout, setUser, fetchWithAuth } = useContext(AuthContext); // <-- fetchWithAuth added
  ...
  const handleSave = async () => {
    ...
    try {
      // Use fetchWithAuth if available, otherwise fallback to fetch
      let res;
      if (fetchWithAuth) {
        res = await fetchWithAuth(`${process.env.REACT_APP_API_URL}/api/users/profile`, {
          method: 'PUT',
          credentials: 'include',
          body: formData
        });
      } else {
        res = await fetch(`${process.env.REACT_APP_API_URL}/api/users/profile`, {
          method: 'PUT',
          credentials: 'include',
          body: formData
        });
      ...
      if (newPassword) {
        ...
        const updatedProfile = fetchWithAuth ? await getProfile(fetchWithAuth) : await getProfile();
